...
namespace ShiftMaster.Employee.Controllers
{
    ...
    public class LeaveBlocksController : ControllerBase
    {
        ...
        [HttpGet("internal/{userId}/active")]
        public async Task<IActionResult> GetActiveLeavesInternal(int userId, [FromServices] ILeaveRepository leaveRepo)
        {
            var leaves = await leaveRepo.GetActiveLeavesAsync(userId);
            return Ok(leaves);
        }

        [HttpGet("internal/{userId}/active-on-date")]
        public async Task<IActionResult> GetActiveLeaveOnDateInternal(int userId, [FromQuery] DateTime date, [FromServices] ILeaveRepository leaveRepo)
        {
            var leaves = await leaveRepo.GetActiveLeavesAsync(userId);
            var leaveOnDate = leaves.FirstOrDefault(l => l.StartDate.Date <= date.Date && l.EndDate.Date >= date.Date);
            if (leaveOnDate == null) return NoContent();
            return Ok(leaveOnDate);
        }
    }
}
