...
namespace ShiftMaster.SchedulingService.Application.Mappers
{
    ...
    .ForMember(dest => dest.CreatedByID, opt => opt.MapFrom((src, dest, destMember, context) =>
    ...
    .ForMember(dest => dest.PublishedDate, opt => opt.MapFrom(src => DateTime.UtcNow));
            ...
            // FIXED: Added explicit string conversion configuration for the Enum status value
