...
public class AttendanceService : IAttendanceService
{
    ...
    {
        var locationExists = await _repo.WorkLocationExistsAsync(locationId);
        if (!locationExists)
        {
    ...
    private void ApplyAttendanceMetrics(AttendanceRecord record, ShiftAssignment assignment)
    {
        ...
        var scheduledHours = assignment.Pattern?.DurationHours ?? CalculateScheduledDurationHours(shiftStart, shiftEnd);
        record.VarianceMinutes = (int)((record.ActualHoursWorked - scheduledHours) * 60);
    }

    private AttendanceStatus DetermineAttendanceStatus(AttendanceRecord record, ShiftAssignment assignment)
    {
        var clockIn = record.ClockIn;
        var clockOut = record.ClockOut;
        ...
        if (!clockIn.HasValue)
        {
            var leave = _repo.GetActiveLeaveForUserDateAsync(record.UserID, record.WorkDate)
                .GetAwaiter()
                .GetResult();

            return leave != null ? AttendanceStatus.Excused : AttendanceStatus.Absent;
        }
    ...
    private static decimal CalculateScheduledDurationHours(TimeSpan shiftStart, TimeSpan shiftEnd)
    {
        ...
        switch (newStatus)
        {
            case TimesheetStatus.SupervisorApproved:
                if (timesheet.Status != TimesheetStatus.Submitted)
                    throw new InvalidWorkflowStateException(
                        "Only submitted timesheets can be supervisor approved"
                    );
            ...
            case TimesheetStatus.HrApproved:
                if (timesheet.Status != TimesheetStatus.SupervisorApproved)
                    throw new InvalidWorkflowStateException(
                        "Only supervisor approved timesheets can be HR approved"
                    );
            ...
            case TimesheetStatus.SentToPayroll:
                if (timesheet.Status != TimesheetStatus.HrApproved)
                    throw new InvalidWorkflowStateException(
                        "Only HR approved timesheets can be sent to payroll"
                    );
            ...
            case TimesheetStatus.Submitted:
                if (timesheet.Status != TimesheetStatus.SupervisorApproved
                    ...
                    throw new InvalidWorkflowStateException(
                        "Only approved timesheets can be returned to submitted"
                    );
